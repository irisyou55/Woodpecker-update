 ...
 */
public class FILEExceptionProcessor extends Executor implements Keyword {
    public FILEExceptionProcessor()
    {
    ...
    public void handle(String keyword, GrammarType type) throws Exception
    {
        ...
        WpLog.recordLog(LogLevelConstant.INFO, "FILE: %s", keyword);
        switch (type)
        {
            case FIRST_GRAMMAR:
     ...
     */
    private void handleFirstGrammar(String keyword) throws IOException {
        String[] parts = Util.removeBlankElement(keyword.split("\\[|;|]"));
        FaultInjection faultInjection=new FaultInjection();
        faultInjection.seizeFILE(keyword);
    }

    /**
     *
     * @param variableName
     * @param keyword Starts with "file["
     * @throws Exception
     */
