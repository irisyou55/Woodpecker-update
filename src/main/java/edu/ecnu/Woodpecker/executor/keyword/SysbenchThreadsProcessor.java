...
public class SysbenchThreadsProcessor extends Executor implements Keyword
{
    public SysbenchThreadsProcessor()
    {
    ...
    public void handle(String keyword, GrammarType type) throws Exception
    {
        ...
        WpLog.recordLog(LogLevelConstant.INFO, "sysbench_threads: %s", keyword);
        switch (type)
        {
            case FIRST_GRAMMAR:
    ...
    /**
     * sysbench关键字处理的地方
     * @param keyword Starts with "sysbench_threads["
     */
    private void handleFirstGrammar(String keyword)
    {
        WpLog.recordLog(LogLevelConstant.INFO, "sysbench_threads 1级文法");
        String[] parts = keyword.split("\\[|;|]");
        String sessionName = parts[1].trim();
        SysbenchTools sysbenchTools = new SysbenchTools();
        if(sessionName.equals("")){
            Session session = null;
            sysbenchTools.seizeSysbenchThreads(session, keyword);
        }
        else {
     ...
     *
     * @param variableName
     * @param keyword Starts with "sysbench["
     * @throws Exception
     */
