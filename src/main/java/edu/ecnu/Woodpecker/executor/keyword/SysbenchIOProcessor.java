...
public class SysbenchIOProcessor extends Executor implements Keyword
{
    public SysbenchIOProcessor()
    {
    ...
    public void handle(String keyword, GrammarType type) throws Exception
    {
        ...
        WpLog.recordLog(LogLevelConstant.INFO, "sysbench_io: %s", keyword);
        switch (type)
        {
            case FIRST_GRAMMAR:
    ...
    /**
     * sysbench关键字处理的地方
     * @param keyword Starts with "sysbench_io["
     */
    private void handleFirstGrammar(String keyword)
    {
        WpLog.recordLog(LogLevelConstant.INFO, "sysbench_io 1级文法");
        String[] parts = keyword.split("\\[|;|]");
        String sessionName = parts[1].trim();
        SysbenchTools sysbenchTools = new SysbenchTools();
        if(sessionName.equals("")){
            Session session = null;
            sysbenchTools.seizeSysbenchIO(session, keyword);
        }
        else {
