...
public class SysbenchCPUProcessor extends Executor implements Keyword
{
    public SysbenchCPUProcessor()
    {
    ...
    public void handle(String keyword, GrammarType type) throws Exception
    {
        ...
        WpLog.recordLog(LogLevelConstant.INFO, "sysbench_cpu: %s", keyword);
        switch (type)
        {
            case FIRST_GRAMMAR:
    ...
    /**
     * sysbench关键字处理的地方
     * @param keyword Starts with "sysbench_cpu["
     */
    private void handleFirstGrammar(String keyword)
    {
        WpLog.recordLog(LogLevelConstant.INFO, "sysbench_cpu 1级文法");
        String[] parts = keyword.split("\\[|;|]");
        String sessionName = parts[1].trim();
        SysbenchTools sysbenchTools = new SysbenchTools();
        if(sessionName.equals("")){
            Session session = null;
            sysbenchTools.seizeSysbenchCPU(session,keyword);
        }
        else {
